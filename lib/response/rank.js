...
var rank = async (e) => {
    ...
    for (const role of sortedRoles) {
        ...
        entries.push({
            ...
            weaponName: detailResp.success && detailResp.data ? (detailResp.data.weaponData?.weapon?.weaponName ?? '-') : '-',
            weaponLevel: detailResp.success && detailResp.data ? (detailResp.data.weaponData?.level ?? 0) : 0,
            weaponStarLevel: detailResp.success && detailResp.data ? (detailResp.data.weaponData?.weapon?.weaponStarLevel ?? 0) : 0,
