    ...
    return (React.createElement(HTML, { style: { width: '1000px' } },
        React.createElement(DarkContainer, null,
            ...
            (queryType === '技能' || queryType === '概览') && skillList && skillList.length > 0 && (React.createElement(Section, { title: '\u6280\u80FD' },
                React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: '2px' } }, skillList.map((entry, i) => {
                    ...
                    return (React.createElement("div", { key: i, style: {
                        ...
                        sk?.iconUrl && React.createElement("img", { src: sk.iconUrl, style: { width: '40px', height: '40px', display: 'block', borderRadius: '8px' } }),
                        React.createElement("div", { style: { flex: 1 } },
                            React.createElement("div", { style: { fontSize: '22px', fontWeight: 'bold', color: '#fff' } }, sk?.name),
                            React.createElement("div", { style: { fontSize: '18px', color: C.textDim } }, sk?.type)),
                        React.createElement("div", { style: { fontSize: '18px', fontWeight: 'bold', color: entry.level >= 10 ? C.gold : '#fff' } },
                            "Lv.",
                            entry.level)));
                })))),
            ...
            queryType === '概览' && weaponData && (React.createElement(Section, { title: '\u88C5\u5907\u6B66\u5668' },
                React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: '16px' } },
                    weaponData.weapon?.weaponIcon && (React.createElement("img", { src: weaponData.weapon.weaponIcon, style: { width: '64px', height: '64px', display: 'block', borderRadius: '10px' } })),
                    React.createElement("div", null,
                        React.createElement("div", { style: { fontSize: '18px', fontWeight: 'bold', color: '#fff' } },
                            weaponData.weapon?.weaponName,
                            React.createElement("span", { style: { color: C.star5, marginLeft: '8px', fontSize: '20px' } }, '★'.repeat(weaponData.weapon?.weaponStarLevel ?? 0))),
